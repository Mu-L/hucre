...
describe("writeChart", () => {
  ...
  it("emits percentStacked grouping for type=line with lineGrouping=percentStacked", () => {
    const result = writeChart(makeChart({ type: "line", lineGrouping: "percentStacked" }), "Sheet1")
  ...
  it("emits percentStacked grouping for type=area with areaGrouping=percentStacked", () => {
    const result = writeChart(makeChart({ type: "area", areaGrouping: "percentStacked" }), "Sheet1")
    expect(result.chartXml).toContain('c:grouping val="percentStacked"')
  })
...
describe("writeChart — axis scale", () => {
  it("emits <c:min> and <c:max> inside <c:scaling> on the value axis", () => {
    const result = writeChart(makeChart({ axes: { y: { scale: { min: 0, max: 100 } } } }), "Sheet1")
    const valAxBlock = result.chartXml.match(/<c:valAx>[\s\S]*?<\/c:valAx>/)![0]
  ...
  it("drops max when min >= max (degenerate range)", () => {
    const result = writeChart(makeChart({ axes: { y: { scale: { min: 10, max: 10 } } } }), "Sheet1")
    const valAxBlock = result.chartXml.match(/<c:valAx>[\s\S]*?<\/c:valAx>/)![0]
    const scalingBlock = valAxBlock.match(/<c:scaling>[\s\S]*?<\/c:scaling>/)![0]
...
describe("writeChart — overlap", () => {
  ...
  it("places <c:gapWidth> before <c:overlap> inside <c:barChart> (OOXML order)", () => {
    ...
    expect(result.chartXml.indexOf("c:gapWidth")).toBeLessThan(result.chartXml.indexOf("c:overlap"))
  })
...
describe("writeChart — scatterStyle", () => {
  ...
  it("falls back to the default lineMarker on an unrecognized scatterStyle", () => {
    ...
    const result = writeChart(makeScatter({ scatterStyle: "bogus" as ChartScatterStyle }), "Sheet1")
    expect(result.chartXml).toContain('c:scatterStyle val="lineMarker"')
...
describe("writeChart — axis noMultiLvlLbl", () => {
  ...
  it("threads the override through bar, column, line, and area chart families", () => {
    for (const type of ["bar", "column", "line", "area"] as const) {
      const result = writeChart(makeChart({ type, axes: { x: { noMultiLvlLbl: true } } }), "Sheet1")
...
describe("writeChart — data table", () => {
  ...
  it("composes underline with fontSize alone (no fontColor) on a self-closing defRPr", () => {
    const result = writeChart(makeChart({ dataTable: { fontSize: 14, underline: true } }), "Sheet1")
  ...
  it("emits a multi-word typeface verbatim", () => {
    const result = writeChart(makeChart({ dataTable: { fontFamily: "Times New Roman" } }), "Sheet1")
  ...
  it("threads fontFamily through every chart family with axes", () => {
    for (const type of ["bar", "column", "line", "area"] as const) {
      const result = writeChart(makeChart({ type, dataTable: { fontFamily: "Verdana" } }), "Sheet1")
  ...
  it("does not leak the dataTable fillColor into plotArea or chartSpace fill slots", () => {
    ...
    // sibling fields.
    const written = writeChart(makeChart({ dataTable: { fillColor: "ABCDEF" } }), "Sheet1").chartXml
    const reparsed = parseChart(written)
  ...
  it("threads borderColor through every chart family with axes", () => {
    for (const type of ["bar", "column", "line", "area"] as const) {
      const result = writeChart(makeChart({ type, dataTable: { borderColor: "1070CA" } }), "Sheet1")
      expect(result.chartXml).toContain(
...
describe("writeChart — legend entries", () => {
  ...
  it("survives a writeXlsx round trip — legendEntries lands in the packaged chart XML", async () => {
    ...
    const out = await writeXlsx({ sheets })
    const chartXml = await extractXml(out, "xl/charts/chart1.xml")
    expect(chartXml).toContain('<c:legendEntry><c:idx val="1"/><c:delete val="1"/></c:legendEntry>')
    const reparsed = parseChart(chartXml)
...
describe("writeChart — axis labelRotation", () => {
  ...
  it("ignores the rotation on pie / doughnut charts (no axes at all)", () => {
    const pie = writeChart(makeChart({ type: "pie", axes: { x: { labelRotation: 45 } } }), "Sheet1")
    const dough = writeChart(
  ...
  it("round-trips a non-default rotation through parseChart", () => {
    const written = writeChart(makeChart({ axes: { x: { labelRotation: 45 } } }), "Sheet1").chartXml
    const reparsed = parseChart(written)
...
describe("writeChart — axis labelFontSize", () => {
  ...
  it("drops non-numeric sizes (string, boolean)", () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const stringy = writeChart(makeChart({ axes: { x: { labelFontSize: "12" as any } } }), "Sheet1")
    const boolish = writeChart(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  it("ignores the size on pie / doughnut charts (no axes at all)", () => {
    const pie = writeChart(makeChart({ type: "pie", axes: { x: { labelFontSize: 12 } } }), "Sheet1")
    const dough = writeChart(
  ...
  it("round-trips a non-default size through parseChart", () => {
    const written = writeChart(makeChart({ axes: { x: { labelFontSize: 12 } } }), "Sheet1").chartXml
    const reparsed = parseChart(written)
...
describe("writeChart — data labels showLeaderLines", () => {
  ...
  it("does not emit <c:showLeaderLines> on a pie chart by default (OOXML default true)", () => {
    ...
    const result = writeChart(makeChart({ type: "pie", dataLabels: { showValue: true } }), "Sheet1")
    const dLbls = dLblsOf(result.chartXml)
...
describe("writeChart — axis labelItalic", () => {
  ...
  it("ignores labelItalic on pie / doughnut charts (no axes at all)", () => {
    const pie = writeChart(makeChart({ type: "pie", axes: { x: { labelItalic: true } } }), "Sheet1")
    const dough = writeChart(
  ...
  it("round-trips labelItalic=true through parseChart", () => {
    const written = writeChart(makeChart({ axes: { x: { labelItalic: true } } }), "Sheet1").chartXml
    const reparsed = parseChart(written)
...
describe("writeChart — axis labelStrike", () => {
  ...
  it("ignores labelStrike on pie / doughnut charts (no axes at all)", () => {
    const pie = writeChart(makeChart({ type: "pie", axes: { x: { labelStrike: true } } }), "Sheet1")
    const dough = writeChart(
  ...
  it("round-trips labelStrike=true through parseChart", () => {
    const written = writeChart(makeChart({ axes: { x: { labelStrike: true } } }), "Sheet1").chartXml
    const reparsed = parseChart(written)
...
describe("writeChart — dataLabels.bold", () => {
  ...
  it('threads bold=false through as b="0" (explicit OOXML default)', () => {
    ...
    const result = writeChart(makeChart({ dataLabels: { showValue: true, bold: false } }), "Sheet1")
    const dLbls = dLblsOf(result.chartXml)
    expect(dLbls).toContain("<c:txPr>")
...
describe("writeChart — titleLayout", () => {
  ...
  it("does not surface titleLayout when the chart hides the title", () => {
    const written = writeChart(makeChart({ showTitle: false }), "Sheet1").chartXml
...
describe("writeChart — axisTitleBorderDash", () => {
  ...
  it("drops axis title dash when no axis title", () => {
    const result = writeChart(makeChart({ axes: { x: { axisTitleBorderDash: "dash" } } }), "Sheet1")
    expect(result.chartXml).not.toContain("<a:prstDash")
  })
